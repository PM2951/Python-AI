...
といった「特徴量設計・モデル選択」の出発点になります。単回帰では相関の高い特徴量から試すのが自然ですし、重回帰でも「相関が高すぎる特徴量を同時に入れない」などの判断材料になります。

また、散布図を描くことで「線形関係か、非線形か」「外れ値がどの程度あるか」を視覚的に確認できます。
...
2. `numeric_cols` を使ってヒートマップを描き、`price` との相関が高い上位 3 つの特徴量を分析してください。
3. `x`, `y`, `z` と `carat` の関係を散布図行列（`sns.pairplot`）で可視化し、「どのペアがほぼ線形な関係に見えるか」を分析してください。
...
2. 1で求めた予測値（y_pred）と実際の値（y_test_s）とのブレを散布図で可視化してください。（完全に当たっていれば x=y の直線上に乗る）
   また、RMSEと決定係数R^2を求めてください。

---

## 7. 重回帰：複数特徴量 → price

現実の問題では、1 つの特徴量だけで目的変数を説明しきれることはほとんどありません。重回帰により、
...
2. 1で求めた予測値（y_pred）と実際の値（y_test_s）とのブレを散布図で可視化してください。（完全に当たっていれば x=y の直線上に乗る）
   また、RMSEと決定係数R^2を求めてください。
3. `coef_` をDataFrameに格納し、上位 20 行をもとに、「価格を上げる方向に効いている特徴量」「下げる方向に効いている特徴量」を分析してください。
