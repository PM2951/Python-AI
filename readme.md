...
* **分位点（quantile）によるフィルタリング**: 上位 1% などを切り落とすことで極端な外れ値を除去できる。
...
1 つのデータだけで学習と評価を同時に行うと、モデルがそのデータに「答えを暗記」してしまい、未知データに対する性能（汎化性能）が全く分からなくなります。  
イメージとしては、過去問を勉強しまくって100点を取ったからと言って本番の試験で100点を出せるとは限らないと思います。ですが、あえて過去1年分を解かずに勉強した上で、置いておいた過去1年分を本番さながら解いた場合の点数は、本番の試験と同じような結果を反映してそうな感じがしますね。
...
にデータを分け、評価用データには「一切触れずに予測モデルを学習し」、最後に評価用データでモデルの予測精度を検証する、という流れが重要です。8:2 はその一例で、データ量やタスクによって割合は調整します。
...
例では説明変数Xをcarat（`X = df[["carat"]]`）としました。carat以外の変数に変換しても良いです。  
じゃあ何を使えばええねんというところですが、判断材料としては相関係数などがあります。  
より高い相関であれば、良い精度を出しやすいです。

### 練習問題

1. 上述のコードを参考に、データのうち１つを目的変数Xとして目的変数（price）との線形回帰モデルを作成し、評価用データの価格の予測値（y_pred）を算出してください
...
2. 説明変数やパラメーターを独自に変化、追加し、高精度なモデルを作成してください。
